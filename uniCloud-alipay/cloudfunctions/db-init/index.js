...
exports.main = async (event, context) => {
  ...
  return { code: 0, msg: '数据库初始化完成', data: { results: results } }
}

async function seedBadges() {
  const collection = db.collection('badges')
  const existing = await collection.count()
  if (existing.total > 0) return { action: 'seedBadges', status: 'skipped', msg: '已有' + existing.total + '个徽章' }
  for (const badge of BADGE_DEFINITIONS) await collection.add(badge)
...
async function seedPurchaseItems() {
  ...
  const existing = await collection.count().catch(function() { return { total: 0 } })
  if (existing.total > 0) return { action: 'seedPurchaseItems', status: 'skipped', msg: '已有' + existing.total + '个商品' }
  for (const item of PURCHASE_ITEMS) await collection.add(item)
