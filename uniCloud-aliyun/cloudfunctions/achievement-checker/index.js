...
async function evaluateCondition(condition, user, session, uid) {
  ...
  switch (type) {
    ...
    case 'sessions_in_day': {
      ...
      const dayStart = getDayStart(session.start_time)
      const dayEnd = dayStart + 86400000
      const countRes = await sessionsCollection
        .where({
          user_id: uid,
          start_time: dbCmd.gte(dayStart).and(dbCmd.lt(dayEnd)),
        })
        .count()
...
async function getBadges(context) {
  ...
  const earned = allBadges.filter(b => earnedKeys.includes(b.key))
  const locked = allBadges.filter(b => !earnedKeys.includes(b.key))
